...
import com.interfaces.client.IClientController;
...
public class ClientController implements IClientController {
    ...
    @Override
    @GetMapping("/period")
    public @ResponseBody ResponseEntity<BaseClientResponseDTO> getForPeriodJson(@org.springframework.web.bind.annotation.RequestBody ClientRequestDTO clientRequestDTO) {
        log.info("{ClientController}");
        log.info("Uri: /rates/period, type: GET, RequestBody: RequestDTO.class");
        Map<BaseClientResponseDTO, HttpStatus> result = clientService.getForPeriod(clientRequestDTO.getStartDate(), clientRequestDTO.getFinishDate());
        ResponseEntity<BaseClientResponseDTO> responseEntity = new ResponseEntity<>(
                (BaseClientResponseDTO) result.keySet().toArray()[0],
                (HttpStatus) result.values().toArray()[0]);
        log.info(responseEntity.toString());
